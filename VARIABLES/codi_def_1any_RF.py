...
# Crear les dades d'entrada per al model
x_weekly, y_weekly = [], []
for i in range(len(combined_data_weekly) - time_steps_weekly):
    x_weekly.append(combined_data_weekly[i:(i + time_steps_weekly)])
    y_weekly.append(labels_weekly[i + time_steps_weekly])
x_weekly, y_weekly = np.array(x_weekly), np.array(y_weekly)
...
rf_model.fit(x_train_weekly.reshape(x_train_weekly.shape[0], -1), y_train_weekly)

# Predicció
rf_predictions = rf_model.predict(x_test_weekly.reshape(x_test_weekly.shape[0], -1))

# Avaluació del model
print("Confusion Matrix:")
print(confusion_matrix(y_test_weekly, rf_predictions))
print("Classification Report:")
print(classification_report(y_test_weekly, rf_predictions))
print("Accuracy Score:")
print(accuracy_score(y_test_weekly, rf_predictions))

# Graficar la precisió del model
plt.figure(figsize=(12, 6))
plt.plot(y_test_weekly, label='True Values')
plt.plot(rf_predictions, label='Predicted Values')
plt.title('True vs Predicted Weekly Bitcoin Price Movements')
plt.legend()
plt.show()
