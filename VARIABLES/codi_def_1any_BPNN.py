# -*- coding: utf-8 -*-
"""
Created on Tue Aug 20 12:29:06 2024
...
# No cal ajustar la finestra temporal amb BPNN; utilitzarem directament les dades setmanals combinades
...
# Crear el model BPNN (Xarxa Neural de Retropropagació)
model_bpn = Sequential()
model_bpn.add(Dense(64, activation='relu', input_shape=(x_train_weekly.shape[1],)))
model_bpn.add(Dropout(0.5))
model_bpn.add(Dense(32, activation='relu'))
model_bpn.add(Dropout(0.5))
model_bpn.add(Dense(1, activation='sigmoid'))
model_bpn.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

# Entrenar el model amb les dades d'entrenament
history_weekly = model_bpn.fit(x_train_weekly, y_train_weekly, epochs=100, batch_size=32, validation_split=0.2, verbose=1)

# Predir amb les dades de prova
predictions_weekly = model_bpn.predict(x_test_weekly)
